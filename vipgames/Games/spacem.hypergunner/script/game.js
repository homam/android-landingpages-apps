...
window.hypergunner.load_game = function (vgApi) {

    var write = function () { if (!!console && !!console.log) console.log.apply(console, arguments); };

    var spritesheet = window.hypergunner.spritesheet;
    var sprite = window.hypergunner.sprite;
    var star = window.hypergunner.star;
    var gamedata = window.hypergunner.gamedata;
    var leveldata = window.hypergunner.leveldata;
    var imglib = window.hypergunner.imglib;
    ...
    function createSprites() {
        ...
        for (var a = 0; a < 4; a++) m.alienbomb[a] = new sprite("alienbomb" + a, "alienbomb", m.spritesheets["alienbombsheet"], 0, 0, 2, 0, 0);
        ...
        initHiScore();
        setTitle();
        g.ticker = setTimeout(window.hypergunner.loop, 0);
    };
    ...
    function init() {
        try {
            g.canvas = document.querySelector('#hypergunnercanvas');
            ...
            setCanvasDimensions();
            g.canvas.style.backgroundColor = "#142760"; //Image = "url('" + g.gamebackdrops[0].src + "')";
    ...
    function initHiScore() {
        ...
        if (typeof localStorage.key == "function") {
            if (localStorage.getItem("hypergunner-hiscore") != null) {
                m.player.hiscore = localStorage.getItem("hypergunner-hiscore");
            }
        }
    };
    ...
    function touch(event) {
        ...
        if (tx < 20 && ty < 20) g.pausemode++; if (g.pausemode > 1) g.pausemode = 0;
    ...
    function setTitle() {
        g.mode = "title";
        //g.canvas.style.backgroundImage = "url('" + g.titlescreen.src + "')";
        g.resetting = 150;
    };
    ...
    function setGame() {
        ...
        g.saucermode = 0;
        m.player.energy = 0;
        m.player.milestohome = 50000000000;
        createStars();
        g.extralife = 0;
    ...
    function setAliens() {
        ...
        if (g.wavetype != 0) {
            for (var row = 0; row < 2; row++) {
                for (var a = 0; a < 4; a++) {
                    var x = 80 + (a * 40);
                    var y = 128 + (row * 40); //(((g.levelwave + 1) * 48) + (row * 24));
                    var d = 0;
                    switch (g.wavetype) {
                        case 1:
                            d = (row == 0) ? 0 : (a < 2) ? 5 : 3;
    ...
    function moveAlien(o) {
        ...
        /*
        if (o.x < 0) changeDirection(o);
        if ((o.x + o.w)> g.canvaswidth) changeDirection(o); 
        if ((o.y - o.h) < 0) changeDirection(o); 
        if ((o.y + o.h)  > (g.canvasheight - 80)) changeDirection(o)
        */
    ...
    function loop() {
        try {
            ...
            switch (g.mode) {
                case "title":
                    hiScore();
                    g.ctx.drawImage(g.titlescreen, 0, (g.canvasheight / 2) - 150);
                ...
                case "landed":
                    draw(m.starfighter);
                    writeText("Welcome home, pilot.", 40, 220, 0);
                    writeText("Planet Earth salutes you.", 40, 236, 0);
                    break;
                case "endlevel":
                    for (var a = 0; a < stars.length; a++) { moveStar(stars[a]); drawStar(stars[a]); }
                    ...
                    if (g.resetting < 1) {
                        g.level++;
                        vgApi.level_end({ level: hypergunner.gameHandler.getLevel() - 1 });
                ...
                case "levelup":
                    ...
                    writeLevel();
                    writeText(m.player.milestohome + " miles to Earth", 32, 260, 0);
                    g.resetting--;
                    if (g.resetting < 1) {
                        setAliens();
                        g.mode = "game";
                        vgApi.level_start({ level: hypergunner.gameHandler.getLevel() });
                    }
                    drawPlayer(m.player);
                    movePlayer(m.player);
                    updateScore();
                ...
                case "gameover":
                    for (var a = 0; a < stars.length; a++) { moveStar(stars[a]); drawStar(stars[a]); }
                    g.ctx.drawImage(g.gameover, 16, 200);
                    g.resetting--;
                    if (g.resetting == 0) {
                    ...
                    writeText(m.player.milestohome + " miles from home", 22, 260, 0);
                    updateScore();
                    break;
            }
            g.ctx.drawImage(g.playpause, g.pausemode * 32, 0, 32, 32, 0, 8, 32, 32);
            if (g.ori != 0) { g.ctx.drawImage(g.notportrait, 0, 64); g.pausemode = 1; }
            if (isNaN(g.framedelay)) g.framedelay = 15;
            g.ticker = setTimeout(window.hypergunner.loop, 15);//g.framedelay);
            //testFPS();
            g.ctx.restore();
        }
        catch (e) {
    ...
    function testFPS() {
        try {
            ...
            if (g.mode != "title" && g.testy < 20) {
                g.testy++; g.ave += c;
                if (g.testy >= 10) {
                    if (g.ave > 1500) g.framedelay = 15; // throttle the fast devices
                }
            }
        }
        catch (e) {
    ...
    function spawnAlienBomb(o) {
        if (rnd(1000) > 10 || !o.visible || g.pausemode > 0 || o.y > (g.canvasheight - 80)) return;
        var d = 4;
        var sp = o.row == 0 ? 8 : 4;
        for (var a = 0; a < m.alienbomb.length; a++) {
    ...
    function hiScore() {
        try {
            //g.ctx.drawImage(g.hiscore, 40, 0);
    ...
    function updateScore() {
        try {
            ...
            if (m.player.score > m.player.hiscore) {
                ...
                localStorage.setItem("hypergunner-hiscore", m.player.hiscore);
            }
            var sScore = new String();
    ...
    function scanInput(e) {
        ...
        //if (keypress > 36 && keypress < 41) { m.player.targetx = m.player.basex; m.player.targety = m.player.basey; } // Keyboard has over-riden the mouse.
        switch (keypress) {
            case 16: // SHIFT
                break;
            case 18: // ALT
            ...
            case 39: // Right
                ...
                //m.player.targetx = g.canvaswidth - 32;
                break;
            case 37: // Left
    ...
    window.hypergunner = window.hypergunner || {};
    window.hypergunner.init = init;
    window.hypergunner.scanInput = scanInput;
    window.hypergunner.stopMove = stopMove;
    window.hypergunner.loop = loop;

    window.hypergunner.gameHandler = {
