...
vipgames.api._internals = {
    ...
    loadCurrentGame: function (gameStartArgs) {
        ...
        var on = function (modifiedEventArgs, originalEventArgs, callback) {
            ...
            setTimeout(function () {
                ...
                if (modifiedEventArgs.event_name == "game_start")
                    data.play_session_id = vipgames.utils.rndstr(12);
        ...
        // note more funcitonlities can be added using
        // 3.speicalFunc (async, in callback), 
        ...
        var getOn = function (name, specialFunc, specialCheckFunc, eventArgsAmplifier) {
            return function (eventArgs, callback) {
                ...
                // if the first argument is a callback, then the function must have been called by one argument.
...
vipgames.api.register = function (game) {
    ...
    var f = function () { vipgames.api._internals.loadCurrentGame({ session_id: vipgames.utils.rndstr(9), player_id: 'player1' }); };
